...
let input = raw.split("\n").map(line => {
    let s = line.split(" ");
    let range = s[0].split("-").map(r => parseInt(r));
    return [range,s[1].substr(0,1),s[2]];
});
...
for(let i = 0; i < input.length; i++){
    let pass = input[i];
    let range = pass[0],letter = pass[1],password = pass[2];

    if(password.length < range[0]) continue;

    let letters = password.split('').filter(l => l == letter);    
    if(letters.length >= range[0] && letters.length <= range[1]) correct++;
...
for(i = 0; i < input.length; i++){
    ...
    let pass = input[i];
    let range = pass[0],letter = pass[1],password = pass[2].split('');

    if(password.length < (range[1] - 1)) continue;

    if(password[range[0] - 1] != password[range[1] - 1] && 
      (password[range[0] - 1] == letter || password[range[1] - 1] == letter)){
        correct++;
    }
}
